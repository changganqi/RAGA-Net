...
#             a_mag, a_phase = torch.fft.fftn(a_vgg[i]).abs() ,torch.fft.fftn(a_vgg[i]).angle()
#             p_mag, p_phase = torch.fft.fftn(p_vgg[i]).abs() ,torch.fft.fftn(p_vgg[i]).angle()
#             n_mag, n_phase = torch.fft.fftn(n_vgg[i]).abs() ,torch.fft.fftn(n_vgg[i]).angle()

#             # Compute magnitue contrastive loss
#             d_ap_mag = self.l1(a_mag, p_mag.detach())
#             if not self.ab:
#                 d_an_mag = self.l1(a_mag, n_mag.detach())
#                 contrastive_mag = d_ap_mag / (d_an_mag + 1e-7)
#             else:
#                 contrastive_mag = d_ap_mag

#             # Compute phase contrastive loss
#             d_ap_phase = self.l1(a_phase, p_phase.detach())
#             if not self.ab:
#                 d_an_phase = self.l1(a_phase, n_phase.detach())
#                 contrastive_phase = d_ap_phase / (d_an_phase + 1e-7)
#             else:
#                 contrastive_phase = d_ap_phase


  
#             # Add magnitude and phase losses
#             loss += self.weights[i] * ((1-self.lamda)*contrastive_mag + self.lamda*contrastive_phase)
