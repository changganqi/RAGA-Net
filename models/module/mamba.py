...
class ExtendedMamba(nn.Module):
    ...
    def forward(self, hidden_states, x_size):
        ...
        batch, seqlen, dim = hidden_states.shape#作用是将输入的张量hidden_states的形状分解为三个变量batch、seqlen和dim，分别表示批次大小、序列长度和特征维度。
        h, w = x_size# x_size是一个包含两个元素的元组或列表，分别表示输入张量在高度和宽度方向上的尺寸。

        def in_proj_function(hidden_states):
            # We do matmul and transpose BLD -> DBL at the same time
            xz = rearrange(
                self.in_proj.weight @ rearrange(hidden_states, "b l d -> d (b l)"),# 作用是将hidden_states张量的形状从(b, l, d)变换为(d, b*l)，然后与in_proj.weight矩阵进行矩阵乘法，最后将结果的形状变换为(d, b, l)。
                "d (b l) -> b d l",
                l=seqlen,
            )# 作用是将矩阵乘法的结果重新变换为(b, d, l)的形状。
            if self.in_proj.bias is not None:
                xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")# 作用是将self.in_proj.bias张量的形状从(d,)变换为(d, 1)，以便与xz张量进行广播操作，从而实现按元素相加。
            
            return xz

        # 1. in_proj
        if self.use_checkpoint:
            xz = checkpoint(in_proj_function, hidden_states, use_reentrant=False)# 通道数分组
        else:
            xz = in_proj_function(hidden_states)# 作用是直接调用in_proj_function函数，计算xz张量。

        if not self.disable_z_branch:
            x, z = xz.chunk(2, dim=1)       # z is the branched part  # 作用是将xz张量在第1维（通道维度）上均分为两个张量x和z。双分支
        else:
            x = xz
            z = None

        # 2. conv
        if self.conv_2d:# 如果self.conv_2d为True，表示使用2D卷积操作。
            x = rearrange(x, 'b d (h w) -> b d h w', h=h, w=w)# 作用是将x张量的形状从(b, d, h*w)变换为(b, d, h, w)，其中h和w分别表示高度和宽度。
        ...
        if self.use_checkpoint:
            ...
            out = self.out_proj(y)  #结尾linnerear层
